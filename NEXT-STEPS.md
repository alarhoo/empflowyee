...
with an intentionally empty initial manifest.
...
2. Add domain-owned Dunder Mifflin seed modules only alongside approved domain migrations; the framework itself creates no workforce records.
3. Integrate persisted development tenant/persona data through the established session contract when the owning domain schema is approved; external production authentication remains separate.
