...
Selected route `/audit/audit-log`; lazy feature `libs/hcm/web/audit/feature-audit-log`. Selection is recorded
...
Search real tenant business action evidence. Use the domain DTO projection, not SELECT * or entity serialization.
...
Search real tenant business action evidence. Rows come from the append-only store with safe DTO projection.
...
| -------------------------- | ----------------------- | --------------------------------------------------------- | ----------------- |
| `GET /api/v1/audit/events` | `hcm.audit.events.read` | `List query: from?,to?,action?,outcome?; actorAccountId?` | `Page<AuditItem>` |
...
`hcm.catalogue.AUDIT_LOG.discover` and is not any permission below.
Actors and subject scope: Tenant administrators.

| Business permission     | Authorized function                          |
| ----------------------- | -------------------------------------------- |
| `hcm.audit.events.read` | Search real tenant business action evidence. |
...
Content columns/fields: Actor ID; action; target type/ID; outcome; timestamp; request correlation.
...
| Project                           | Root                                   | Tags                                                                |
| --------------------------------- | -------------------------------------- | ------------------------------------------------------------------- |
| `hcm-web-audit-feature-audit-log` | `libs/hcm/web/audit/feature-audit-log` | `product:hcm`, `runtime:web`, `domain:audit`, `type:feature`        |
...
Branch `codex/hcm-1-audit-log` after all 20 local designs have reviewed evidence.
