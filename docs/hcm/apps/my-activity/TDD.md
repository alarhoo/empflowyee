...
Floorplan `UX-FP-DYNAMIC-PAGE`, mode **NATIVE**. Use existing HcmDynamicPage with persistent title/actions and collapsible filter/scope context; the native responsive table displays the complete self-safe projection. There is no separate object detail or dialog.
...
The audit-owned reader reuses stable business-event paging, binds cursors to the
self projection and verified actor, and returns only explicit self DTO fields.
The module supplies independently authorized tenant/self transaction callbacks.
No operator reason, actor identifier or request correlation reaches self clients.
