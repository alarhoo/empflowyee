# Data Export Log — technical design

Status: complete for review; no implementation/admission or revision approval claimed.

## ROUTE

Selected route `/audit/data-export-log`; lazy feature `libs/hcm/web/audit/feature-data-export-log`. Selection is recorded
...
Inspect actual export events without implementing export generation. Use the domain DTO projection, not SELECT * or entity serialization.
...
| --------------------------- | ------------------------ | --------------------------------------------------------- | ------------------ |
| `GET /api/v1/audit/exports` | `hcm.audit.exports.read` | `List query: from?,to?,action?,outcome?; actorAccountId?` | `Page<ExportItem>` |
...
`hcm.catalogue.DATA_EXPORT_LOG.discover` and is not any permission below.
Actors and subject scope: Tenant administrators.

| Business permission      | Authorized function                                                  |
| ------------------------ | -------------------------------------------------------------------- |
| `hcm.audit.exports.read` | Inspect actual export events without implementing export generation. |
...
Floorplan `UX-FP-DYNAMIC-PAGE`, mode **NATIVE**. Use existing HcmDynamicPage with persistent title/actions and collapsible filter/scope context; the native responsive table displays the complete safe export projection; there is no separate detail workflow or dialog.
...
Content columns/fields: Export action; actor; target type/ID; outcome; timestamp.
...
| Project                                 | Root                                         | Tags                                                                |
| --------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------- |
| `hcm-web-audit-feature-data-export-log` | `libs/hcm/web/audit/feature-data-export-log` | `product:hcm`, `runtime:web`, `domain:audit`, `type:feature`        |
...
The export action registry is intentionally empty until a real producer and its
event schema are separately reviewed. SQL still queries the tenant-owned store,
restricted to category export and registered actions; unregistered inserted rows
are not accepted as evidence. Action/outcome query values are rejected while no
values are registered, and the UI omits empty selectors. Date, actor, sort and
bounded pagination controls remain available. No export producer is invented.

## DEPENDENCIES

Audit append store; no export producer is added in this local stage.
...
Branch `codex/hcm-1-data-export-log` after all 20 local designs have reviewed evidence.
