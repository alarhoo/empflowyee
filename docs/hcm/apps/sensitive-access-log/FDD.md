# Sensitive Access Log — functional design

Status: complete for review; approved local business scope, document revision not yet approved.

App `SENSITIVE_ACCESS_LOG`; owner `audit`; HCM-1 local stage.

## SCOPE

Inspect document access metadata without granting document-content access.
...
2. Inspect document access metadata without granting document-content access. Rows come from the append-only store with safe DTO projection.
3. Filter and paginate using stable event ordering; no raw payload, document body or credential fields.
4. Unknown stream completion is shown honestly, never as a completed download.

<a id="req-sensitive-access-log-001"></a>

## REQ-SENSITIVE-ACCESS-LOG-001 — Read scoped evidence
...
<a id="req-sensitive-access-log-002"></a>

## REQ-SENSITIVE-ACCESS-LOG-002 — Inspect bounded results
...
<a id="req-sensitive-access-log-003"></a>

## REQ-SENSITIVE-ACCESS-LOG-003 — Handle absent or incomplete evidence
...
<a id="req-sensitive-access-log-004"></a>

## REQ-SENSITIVE-ACCESS-LOG-004 — Authorize independently
...
<a id="req-sensitive-access-log-005"></a>

## REQ-SENSITIVE-ACCESS-LOG-005 — Accessible truthful states
...
<a id="req-sensitive-access-log-006"></a>

## REQ-SENSITIVE-ACCESS-LOG-006 — Persist and attribute correctly
...
Display: Actor; document target ID; authorization/stream phase; related event; timestamp.
