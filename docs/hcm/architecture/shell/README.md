...
4. Select **No tenant override** to remove the tenant overlay. HER restores its supplied palette; Horizon restores native SAP emphasis values.
5. Expand **Mock session — roles and entitlements**. Toggle roles and licensed capabilities, then inspect **Spaces** and **Browse apps**.
6. Choose each preview: **Dynamic Page** and **Object Page**.
...
| `hcm-web-ux-feature-theme-lab`               | `libs/hcm/web/ux/feature-theme-lab` | Lazy controls and shared production Dynamic/Object Page examples               |
...
sequenceDiagram
    ...
    Theme->>DOM: Apply native HER palette and optional tenant accent
```
...
Unit tests cover catalog role/entitlement rules, pruning, fixture updates, invalid colors, WCAG contrast calculations, complete overlay cleanup, asset failures and rapid theme selections. Browser tests cover all eight theme/preview combinations, native SAP background changes, branding controls, catalog changes, and 390px layouts. Keep a visual review of both production previews in the acceptance pass; these checks do not constitute a complete accessibility audit.
...
## Floorplan workshop and next milestone

The [curated Storybook](../../ux/storybook.md) and Theme Lab now share the production Dynamic Page and Object Page examples. Other generated patterns are deferred pending capability and UX acceptance. Consult the [capability matrix](../../ux/floorplans/component-capability-matrix.md) and [composition API guide](../../ux/floorplans/README.md) before feature integration. Authentication/bootstrap, real locale switching, persistence and domain transactions remain separate work. Follow [next steps](../../../../NEXT-STEPS.md).
