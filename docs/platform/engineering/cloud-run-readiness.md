...
The initial private deployment verified all seven app responses and health endpoints with authenticated HTTP requests and produced clean drift checks. The subsequent correction in PR #5 applied the four web invocation changes and passed normal-browser and anonymous page/configuration/asset/health checks. All three APIs still reject anonymous requests and accept authorized requests; the fresh DEV Cloud Run plan reports no drift. See [service URLs, revision evidence and access instructions](dev-deployment.md). Re-plan before future infrastructure changes; saved plans are time-sensitive and Git-ignored.
