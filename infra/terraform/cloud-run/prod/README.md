# PROD Cloud Run service stack

Creates seven private Cloud Run service shells in `empflowyee-prd` using the reusable module.

This stack is separate from `infra/terraform/environments/prod`, which owns WIF, deployer IAM and runtime service accounts.
...
Do not run this apply concurrently with a PROD application deployment.
