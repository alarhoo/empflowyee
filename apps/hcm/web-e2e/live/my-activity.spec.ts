...
async function openActivity(page: Page): Promise<void> {
	...
	await page.getByRole('textbox', { name: 'Search applications' }).fill('MY_ACTIVITY')
	await page.getByRole('button', { name: 'My Activity — Available', exact: true }).click()
	await expect(page.getByRole('grid', { name: 'Activity events', exact: true })).toBeVisible()
	await expect(page.locator('ef-hcm-my-activity').getByText('Loading your activity…')).toHaveCount(
		0,
	)
}

test('TEST-MY-ACTIVITY-005 shows own safe history and preserves query errors', /** Read real self data, filter, retry and switch personas without exposing tenant diagnostic fields. */ async ({
	page,
	context,
}) => {
	await openActivity(page)
	const view = page.locator('ef-hcm-my-activity')
	await expect(
		view.getByRole('gridcell', { name: 'role.created', exact: true }).first(),
	).toBeVisible()
	await expect(view.getByRole('textbox', { name: /Actor/ })).toHaveCount(0)
	await expect(view.getByText('Request correlation', { exact: true })).toHaveCount(0)
	await expect(view.getByText(/Reason: Browser/)).toHaveCount(0)
	await view
		.getByRole('textbox', { name: 'Activity from', exact: true })
	...
	await view.getByRole('textbox', { name: 'Activity to', exact: true }).fill('2039-01-01T00:00:00Z')
	await view.getByRole('button', { name: 'Apply filters', exact: true }).click()
	await expect(view.getByRole('note', { name: /Enter valid ISO date-times/ })).toBeVisible()
	await view.getByRole('textbox', { name: 'Activity to', exact: true }).fill('')
	await view.getByRole('textbox', { name: 'Activity from', exact: true }).fill('')
	await context.setOffline(true)
	await view.getByRole('button', { name: 'Apply filters', exact: true }).click()
	await expect(view.getByRole('note', { name: /Your activity could not be loaded/ })).toBeVisible()
	await context.setOffline(false)
	await view.getByRole('button', { name: 'Retry activity', exact: true }).click()
	await expect(
		view.getByRole('gridcell', { name: 'role.created', exact: true }).first(),
	).toBeVisible()
	await page
	...
	await page.getByRole('textbox', { name: 'Search applications' }).fill('MY_ACTIVITY')
	await page.getByRole('button', { name: 'My Activity — Available', exact: true }).click()
	await expect(view.getByRole('gridcell', { name: 'role.created', exact: true })).toHaveCount(0)
	await expect(view.getByRole('button', { name: /Export|Delete|Edit/ })).toHaveCount(0)
})
test('keeps the native self-activity filters and table accessible across themes and sizes', /** Check real responsive activity rows without feature CSS overrides. */ async ({
	page,
}) => {
	...
	for (const [variant, label] of [
		...
		for (const width of [390, 768, 1440, 2560]) {
			...
			expect(
				(await new AxeBuilder({ page }).include('ef-hcm-my-activity').analyze()).violations,
			).toEqual([])
			expect(
				await page.evaluate(
			...
			await expect(
				page
					.locator('ef-hcm-my-activity')
					.getByRole('grid', { name: 'Activity events', exact: true }),
			).toBeVisible()
		}
		await page.setViewportSize({ width: 1440, height: 1000 })
		await page.screenshot({ path: `.tmp/hcm-my-activity/${variant}.png`, fullPage: true })
	}
})
test('applies and removes persisted tenant branding on the real activity screen', /** Temporarily change only local tenant presentation and restore its exact prior value even after failure. */ async ({
	page,
}) => {
