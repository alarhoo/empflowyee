import { KyselyDocumentWorkforce } from './workforce-reader'
import { TemplateFileUnitOfWork } from '@empflowyee/hcm-api-documents-application'
import { queryKey, after, page } from './document-pages'
import { DocumentReservations } from './document-reservations'
import type { DocumentDownloadAuditEvent } from '@empflowyee/hcm-api-audit-application'
import { createHash, randomUUID } from 'node:crypto'
import { sql } from 'kysely'
import {
	DocumentError,
	type WorkerDocument,
	type WorkerDocumentVersion,
	type WorkerDocumentQuery,
	type WorkerQuery,
	type WorkerDocumentAppend,
	type VersionQuery,
	type DocumentPage,
	type WorkerUploadResult,
	type DocumentTypeQuery,
} from '@empflowyee/hcm-documents-contract'
import {
	WorkerFileUnitOfWork,
	type WorkerFileRepository,
	type WorkerIntent,
	type WorkerReservation,
	type DocumentFile,
} from '@empflowyee/hcm-api-documents-application'
import {
	requireDocumentRevision,
	requireEnabledDocumentType,
} from '@empflowyee/hcm-api-documents-domain'
import {
	HcmAccessDatabase,
	type AuthorizedAccessWork,
} from '@empflowyee/hcm-api-access-control-infrastructure'
import { type AuthenticatedHcmContext } from '@empflowyee/hcm-api-runtime-application'
import { KyselyDocumentTypes } from './hcm-api-documents-infrastructure'

const documentColumns = sql`id,worker_id AS "workerId",type_id AS "typeId",label,revision`
const versionColumns = sql`v.id,v.employee_visible AS "employeeVisible",v.revision,v.version_number AS "versionNumber",b.safe_filename AS filename,b.media_type AS "mediaType",b.byte_length AS "byteLength",to_char(v.created_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"') AS "createdAt"`
class WorkerRepository implements WorkerFileRepository {
	/** Bind documents, audit and all SQL to the same verified transaction. */
	constructor(private readonly scope: AuthorizedAccessWork) {}
	/** Query a stable label/id page with literal search and explicit tenant ownership. */
	async list(query: WorkerDocumentQuery): Promise<DocumentPage<WorkerDocument>> {
		const binding = queryKey(this.scope, query),
			position = after(query.cursor, binding),
			ascending = query.sort === 'label:asc',
			order = ascending ? sql`ASC` : sql`DESC`,
			compare = ascending ? sql`>` : sql`<`
		if (position && typeof position.position !== 'string')
			throw new DocumentError('invalid-request')
		const search = '%' + query.q.replace(/[\\%_]/g, '\\$&') + '%'
		const rows = (
			await sql<WorkerDocument>`SELECT ${documentColumns} FROM hcm.employee_document WHERE tenant_id=${this.scope.actor.tenantId} ${query.q ? sql`AND label ILIKE ${search}` : sql``} ${query.workerId ? sql`AND worker_id=${query.workerId}` : sql``} ${query.typeId ? sql`AND type_id=${query.typeId}` : sql``} ${position ? sql`AND (label COLLATE "C",id COLLATE "C") ${compare} (${position.position} COLLATE "C",${position.id} COLLATE "C")` : sql``} ORDER BY label COLLATE "C" ${order},id COLLATE "C" ${order} LIMIT ${query.limit + 1}`.execute(
				this.scope.transaction,
			)
		).rows
		return page(
			rows,
			query.limit,
			binding,
			/** Continue from the final visible label/id tuple. */ (row) => row.label,
		)
	}
	/** Conceal foreign aggregates independently of row-level security. */
	async get(id: string): Promise<WorkerDocument> {
		const row = (
			await sql<WorkerDocument>`SELECT ${documentColumns} FROM hcm.employee_document WHERE tenant_id=${this.scope.actor.tenantId} AND id=${id}`.execute(
				this.scope.transaction,
			)
		).rows[0]
		if (!row) throw new DocumentError('not-found')
		return row
	}
	/** Return only committed versions, keeping storage identities private. */
	async versions(id: string, query: VersionQuery): Promise<DocumentPage<WorkerDocumentVersion>> {
		await this.get(id)
		const binding = queryKey(this.scope, query, id),
			position = after(query.cursor, binding)
		if (position && typeof position.position !== 'number')
			throw new DocumentError('invalid-request')
		const rows = (
			await sql<WorkerDocumentVersion>`SELECT ${versionColumns} FROM hcm.employee_document_version v JOIN hcm.document_blob b ON b.tenant_id=v.tenant_id AND b.id=v.blob_id WHERE v.tenant_id=${this.scope.actor.tenantId} AND v.document_id=${id} AND b.state='Ready' ${position ? sql`AND (v.version_number,v.id)<(${position.position},${position.id})` : sql``} ORDER BY v.version_number DESC,v.id DESC LIMIT ${query.limit + 1}`.execute(
				this.scope.transaction,
			)
		).rows
		return page(
			rows,
			query.limit,
			binding,
			/** Continue in immutable version order. */ (row) => row.versionNumber,
		)
	}
	/** Consume the workforce-owned identity projection without account creation. */
	workers(query: WorkerQuery) {
		return new KyselyDocumentWorkforce(this.scope).listWorkers(query)
	}
	/** Change only the selected version, atomically with audit and actor-bound retry evidence. */
	async share(
		id: string,
		versionId: string,
		value: WorkerDocumentAppend,
		key: string,
		requestId: string,
	): Promise<WorkerDocumentVersion> {
		const tenant = this.scope.actor.tenantId,
			actor = this.scope.actor.accountId,
			operation = 'worker-share',
			hash = createHash('sha256')
				.update(JSON.stringify([id, versionId, value]))
				.digest('hex')
		const prior = (
			await sql<{
				hash: string
				response: WorkerDocumentVersion
			}>`SELECT request_hash AS hash,response FROM hcm.document_command_receipt WHERE tenant_id=${tenant} AND actor_account_id=${actor} AND operation=${operation} AND idempotency_key=${key}::uuid`.execute(
				this.scope.transaction,
			)
		).rows[0]
		if (prior) {
			if (prior.hash !== hash) throw new DocumentError('idempotency-conflict')
			return prior.response
		}
		await this.get(id)
		const found = (
			await sql<{
				revision: number
			}>`SELECT revision FROM hcm.employee_document_version WHERE tenant_id=${tenant} AND document_id=${id} AND id=${versionId}`.execute(
				this.scope.transaction,
			)
		).rows[0]
		if (!found) throw new DocumentError('not-found')
		requireDocumentRevision(found.revision, value.expectedRevision)
		await sql`UPDATE hcm.employee_document_version SET employee_visible=${value.employeeVisible},revision=revision+1 WHERE tenant_id=${tenant} AND document_id=${id} AND id=${versionId} AND revision=${value.expectedRevision}`.execute(
			this.scope.transaction,
		)
		const result = (
			await sql<WorkerDocumentVersion>`SELECT ${versionColumns} FROM hcm.employee_document_version v JOIN hcm.document_blob b ON b.tenant_id=v.tenant_id AND b.id=v.blob_id WHERE v.tenant_id=${tenant} AND v.document_id=${id} AND v.id=${versionId} AND b.state='Ready'`.execute(
				this.scope.transaction,
			)
		).rows[0]
		if (!result) throw new DocumentError('not-found')
		await this.scope.audit.append({
			action: 'document.visibility-changed',
			targetId: versionId,
			requestId,
			summary: { reason: value.reason, changedFields: ['employeeVisible'] },
		})
		await sql`INSERT INTO hcm.document_command_receipt(tenant_id,actor_account_id,operation,idempotency_key,request_hash,response) VALUES(${tenant},${actor},${operation},${key}::uuid,${hash},${JSON.stringify(result)}::jsonb)`.execute(
			this.scope.transaction,
		)
		return result
	}
	/** Reuse classification queries while projecting only approved picker metadata. */
	async types(query: DocumentTypeQuery) {
		const result = await new KyselyDocumentTypes(this.scope).list({ ...query, enabled: true })
		return {
			...result,
			items: result.items.map(
				/** Omit classification internals from picker responses. */ (row) => ({
					id: row.id,
					code: row.code,
					label: row.label,
				}),
			),
		}
	}
	/** Allow append on an existing worker document even if its type was subsequently disabled. */
	async validate(intent: WorkerIntent): Promise<void> {
		if (intent.kind === 'worker-create') {
			await new KyselyDocumentWorkforce(this.scope).requireWorker(intent.value.workerId)
			requireEnabledDocumentType(await new KyselyDocumentTypes(this.scope).get(intent.value.typeId))
		} else
			requireDocumentRevision(
				(await this.get(intent.targetId)).revision,
				intent.value.expectedRevision,
			)
	}
	/** Read actor-bound upload evidence through the shared reservation adapter. */
	find(operation: string, key: string) {
		return new DocumentReservations<WorkerIntent, WorkerUploadResult>(this.scope).find(
			operation,
			key,
		)
	}
	/** Reserve bytes through the shared storage metadata writer. */
	reserve(intent: WorkerIntent, key: string, hash: string, file: DocumentFile) {
		return new DocumentReservations<WorkerIntent, WorkerUploadResult>(this.scope).reserve(
			intent,
			key,
			hash,
			file,
		)
	}
	/** Finalize immutable bytes and a version exactly once with attributed audit and receipt. */
	async finish(
		intent: WorkerIntent,
		key: string,
		reservation: WorkerReservation,
		requestId: string,
	): Promise<WorkerUploadResult> {
		const tenant = this.scope.actor.tenantId,
			actor = this.scope.actor.accountId,
			id = reservation.aggregateId
		let document: WorkerDocument
		if (intent.kind === 'worker-create') {
			document = (
				await sql<WorkerDocument>`INSERT INTO hcm.employee_document(tenant_id,id,worker_id,type_id,label,revision,created_by_account_id) VALUES(${tenant},${id},${intent.value.workerId},${intent.value.typeId},${intent.value.label},1,${actor}) RETURNING ${documentColumns}`.execute(
					this.scope.transaction,
				)
			).rows[0]
		} else {
			document = (
				await sql<WorkerDocument>`UPDATE hcm.employee_document SET revision=revision+1 WHERE tenant_id=${tenant} AND id=${id} AND revision=${intent.value.expectedRevision} RETURNING ${documentColumns}`.execute(
					this.scope.transaction,
				)
			).rows[0]
			if (!document) throw new DocumentError('revision-conflict')
		}
		const blob = (
			await sql<{
				id: string
			}>`UPDATE hcm.document_blob SET state='Ready' WHERE tenant_id=${tenant} AND storage_key=${reservation.file.key}::uuid AND state='Staged' RETURNING id`.execute(
				this.scope.transaction,
			)
		).rows[0]
		if (!blob) throw new DocumentError('upload-failed')
		const versionId = randomUUID()
		await sql`INSERT INTO hcm.employee_document_version(tenant_id,id,document_id,version_number,blob_id,employee_visible,created_by_account_id) VALUES(${tenant},${versionId},${id},${document.revision},${blob.id}::uuid,${intent.value.employeeVisible},${actor})`.execute(
			this.scope.transaction,
		)
		const version = (
			await sql<WorkerDocumentVersion>`SELECT ${versionColumns} FROM hcm.employee_document_version v JOIN hcm.document_blob b ON b.tenant_id=v.tenant_id AND b.id=v.blob_id WHERE v.tenant_id=${tenant} AND v.id=${versionId}`.execute(
				this.scope.transaction,
			)
		).rows[0]
		await this.scope.audit.append({
			action: 'document.worker-version-added',
			targetId: id,
			requestId,
			summary: { reason: intent.value.reason, changedFields: ['version'] },
		})
		const result = { document, version }
		await sql`INSERT INTO hcm.document_command_receipt(tenant_id,actor_account_id,operation,idempotency_key,request_hash,response) VALUES(${tenant},${actor},${intent.kind},${key}::uuid,${reservation.payloadHash},${JSON.stringify(result)}::jsonb)`.execute(
			this.scope.transaction,
		)
		await sql`UPDATE hcm.document_upload_attempt SET state='Ready' WHERE tenant_id=${tenant} AND id=${reservation.id}::uuid AND state='Staged'`.execute(
			this.scope.transaction,
		)
		return result
	}
	/** Resolve a Ready attachment through its exact parent and tenant; no storage-key endpoint exists. */
	async download(id: string, versionId: string): Promise<DocumentFile> {
		const file = (
			await sql<DocumentFile>`SELECT b.storage_key AS key,b.sha256,b.byte_length AS "byteLength",b.media_type AS "mediaType",b.safe_filename AS filename FROM hcm.employee_document_version v JOIN hcm.document_blob b ON b.tenant_id=v.tenant_id AND b.id=v.blob_id WHERE v.tenant_id=${this.scope.actor.tenantId} AND v.document_id=${id} AND v.id=${versionId} AND b.state='Ready'`.execute(
				this.scope.transaction,
			)
		).rows[0]
		if (!file) throw new DocumentError('not-found')
		return file
	}
	/** Use the audit-owned append port for both authorization and observed outcome. */
	auditDownload(
		id: string,
		requestId: string,
		phase: 'Authorized' | 'Completed' | 'Failed',
		relatedId?: string,
	): Promise<string> {
		let action: DocumentDownloadAuditEvent['action'] = 'document.download-authorized'
		if (phase === 'Completed') action = 'document.download-completed'
		if (phase === 'Failed') action = 'document.download-failed'
		return this.scope.audit.append({
			action,
			targetId: id,
			targetType: 'employee-document-version',
			requestId,
			relatedEventId: relatedId ?? null,
			summary: {},
		})
	}
}
export class KyselyWorkerFileUnit extends WorkerFileUnitOfWork {
	/** Reuse current authority and the shared terminal reservation cleanup. */ constructor(
		private readonly database: HcmAccessDatabase | null,
		private readonly reservations: TemplateFileUnitOfWork,
	) {
		super()
	}
	/** Bind worker-document metadata and audit to one verified transaction. */
	execute<T>(
		context: AuthenticatedHcmContext,
		permission: string,
		write: boolean,
		work: (repo: WorkerFileRepository) => Promise<T>,
	): Promise<T> {
		if (!this.database) throw new DocumentError('storage-unavailable')
		return this.database.execute(
			context,
			{ permission: 'hcm.documents.' + permission, entitlement: 'hcm.documents' },
			write,
			/** Compose document-owned queries on the same executor. */ (scope) =>
				work(new WorkerRepository(scope)),
		)
	}
	/** Mark only the current actor's previously reserved attempt failed. */ fail(
		context: AuthenticatedHcmContext,
		id: string,
	) {
		return this.reservations.fail(context, id)
	}
}
