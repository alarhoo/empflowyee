...
import { WorkerShareDialogComponent } from './worker-share-dialog.component'
...
@Component({
	...
	imports: [
		...
		MessageStrip,
		FormField,
		WorkerUploadComponent,
		WorkerShareDialogComponent,
	],
...
export class EmployeeDocumentsComponent {
	...
	readonly versions = signal<WorkerDocumentVersion[]>([])
	readonly versionCursor = signal<string | null>(null)
	readonly versionBusy = signal(false)
	readonly versionMessage = signal('')
	readonly downloading = signal<string | null>(null)
	readonly shareTarget = signal<WorkerDocumentVersion | null>(null)
	readonly shareDialog = viewChild(WorkerShareDialogComponent)
	readonly uploadTarget = signal<WorkerDocument | null>(null)
	readonly canDownload = computed(
		/** Reflect the independent download capability. */ () =>
			this.runtime.context()?.access.permissions.includes('hcm.documents.worker.download') === true,
	)
	readonly detailActions = computed(
		/** Keep detail navigation and focused upload actions explicit. */ () => [
			{ id: 'back', label: 'Back to documents' },
			...(this.canManage() && this.selected()
				? [{ id: 'append', label: 'Upload new version', mutates: true }]
				: []),
		],
	)
	private applied: WorkerDocumentQuery = { q: '', sort: 'label:asc', limit: 25 }
	readonly filters = signal({ q: '', typeId: '', workerId: '', sort: 'label:asc' })
	readonly fields = form(this.filters)
	readonly rows = signal<WorkerDocument[]>([])
	readonly cursor = signal<string | null>(null)
	readonly state = signal<HcmPageState>('loading')
	readonly busy = signal(false)
	readonly message = signal('')
	readonly editing = signal(false)
	readonly selected = signal<WorkerDocument | null>(null)
	readonly dialog = viewChild(WorkerUploadComponent)
	readonly canRead = computed(
		/** Entry visibility is separate from document-content authority. */ () =>
			this.runtime.context()?.access.permissions.includes('hcm.documents.worker.read') === true,
	)
	...
	readonly actions = computed(
		/** Offer only the approved worker document creation command. */ () =>
			this.canRead() && this.canManage()
				? [{ id: 'create', label: 'Upload employee document', mutates: true }]
				: [],
	)
			...
			/** Track context replacement only. */ () => {
				...
				untracked(
					/** Reset caller-specific state without subscribing the effect to drafts. */ () => {
						...
						this.fields().reset({ q: '', typeId: '', workerId: '', sort: 'label:asc' })
						this.load()
					},
				)
			},
	...
	load(): void {
		...
		try {
			...
			this.applied = parseWorkerDocumentQuery(params)
		} catch {
	...
	/** Open a focused new-worker document or version-upload action. */
	edit(item: WorkerDocument | null = null): void {
	...
	/** Reload committed data and select the new or versioned worker document. */
	...
	canLeave(): Promise<boolean> {
		return this.shareDialog()?.canLeave() ?? this.dialog()?.canLeave() ?? Promise.resolve(true)
	}
	/** Open a role-independent worker document deep link in the native mid column. */
	...
	download(version: WorkerDocumentVersion): void {
		const id = this.selectedId()
