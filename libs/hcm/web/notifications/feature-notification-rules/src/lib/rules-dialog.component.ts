...
import { parseRuleSave, type NotificationRule } from '@empflowyee/hcm-notifications-contract'
...
@Component({
	selector: 'ef-hcm-rules-dialog',
	imports: [Text, Dialog, Form, FormItem, Label, TextArea, Button, Bar, MessageStrip, FormField],
	templateUrl: './rules-dialog.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RuleDialogComponent implements OnInit, OnDestroy {
	readonly action = input.required<NotificationRule>()
	...
	title(): string {
		return this.action().enabled ? 'Disable rule' : 'Enable rule'
	}
	...
	save(): void {
		...
		try {
			body = parseRuleSave({
				...
				expectedRevision: this.action().revision,
			})
		} catch {
		...
		this.error.set('')
		this.api
			.saveRule(this.action().eventType, body, this.attempt.key)
