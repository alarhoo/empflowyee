...
export class MyActivityComponent {
	...
	readonly rows = signal<MyActivityItem[]>([])
	...
	load(more = false): void {
		...
		this.request = this.api
			.activity({ ...this.applied, ...(more ? { cursor: this.cursor() ?? '' } : {}) })
				...
				error: /** Keep filters and explain denied or failed reads. */ (error: unknown) => {
					this.error.set(
						error instanceof HttpErrorResponse && [401, 403].includes(error.status)
							? 'This session no longer has permission to read your activity.'
							: 'Your activity could not be loaded. Retry the current filters.',
					)
					this.pending.set(false)
					this.state.set('content')
				},
	...
	summary(item: MyActivityItem): string {
