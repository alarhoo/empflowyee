...
import { parseExportQuery, type ExportItem } from '@empflowyee/hcm-audit-contract'
...
@Component({
	...
	templateUrl: './data-export-log.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DataExportLogComponent {
	...
	readonly rows = signal<ExportItem[]>([])
	...
	load(more = false): void {
		...
		this.request = this.api
			.exports({ ...this.applied, ...(more ? { cursor: this.cursor() ?? '' } : {}) })
				...
				error: /** Keep filters and explain denied or failed reads. */ (error: unknown) => {
					this.error.set(
						error instanceof HttpErrorResponse && [401, 403].includes(error.status)
							? 'This session no longer has permission to read the export log.'
							: 'Export events could not be loaded. Retry the current filters.',
					)
					this.pending.set(false)
					this.state.set('content')
				},
